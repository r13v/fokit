...
The reference implementation relied on Tailwind grid classes, while Form, Please
definitions must remain portable across CSS frameworks, design systems, and
page, modal, and sidebar containers. Form, Please now also ships default structural
slots so a generated form can render before an application implements a full
slot set.

## Decision

Form, Please keeps visual styling in application-owned form kits, stores only finite
...
component library. Consumers may explicitly import `form-please/layout.css` for
...
- A mandatory built-in theme would expand the Form, Please UI compatibility surface and
...
- `form-please/layout.css` contains only grid, spacing, spans, and container-query
