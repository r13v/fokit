...
export function normalizeDefinition<Schema extends StandardSchema>(
	...
	}) as NormalizedDefinition<Schema>
}
	...
	return Object.freeze(
		source.map((candidate) => {
			...
			if (kind === "field") {
				...
				if (candidate.valuePolicy === "unset") {
					throw new TypeError(
						'TanStack form does not support valuePolicy "unset"',
					)
				}
			}
			...
			const node = Object.freeze({
				...
				...(kind === "field" ? { valuePolicy: "preserve" } : {}),
			}) as RuntimeNode
...
function resolveOptional<Context>(
	...
	const resolverValues = new Proxy(
		Object.create(null) as Record<string, unknown>,
		{
			get: (_target, property) => {
				if (typeof property !== "string") {
					throw new TypeError("UI resolver paths must be strings")
				}
				return getPathValue(
					values,
					joinPath(pathPrefix, normalizePath(property)),
				)
			},
			set: () => {
	...
	const result = (
		value as (
			resolverValues: Readonly<Record<string, unknown>>,
			details: { readonly context: Readonly<Context> },
		) => unknown
	)(resolverValues, { context })
	if (
		result !== null &&
