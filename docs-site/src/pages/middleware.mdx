...
Middleware is form-local. It does not create another form store. React Hook
Form remains the owner of editable values and form state.
...
The callbacks use their latest React render versions. Their transactions are
readonly views rather than archival snapshots. Copy retained audit or save data.
Use middleware when several rules must compose in a declared order.
...
| `source` | The generated control, generated array action, `form.update`, or optional history restore. |
...
`toIndex`. Optional managed history restores contain an `undo`, `redo`, `seek`,
or `import` action.
