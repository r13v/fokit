...
`kit.defineFragment(schema, { ui })` defines one schema-owned UI subtree and
keeps the concrete schema available as `fragment.schema`. Compose that schema
into a form schema, then place the fragment at each compatible object path:
