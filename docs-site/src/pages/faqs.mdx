...
`kit.Form` captures the editable input before raw RHF submission. The resolver
then supplies transformed `FormOutput<Schema>` to the same submit attempt.
...
errors](/advanced#apply-server-field-errors) for a typed `setError` example.
...
Use `form.api.setValue` when the editable value must be cleared as soon as
