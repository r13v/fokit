...
| --- | --- |
| TanStack Form | Stores selected IDs and field metadata. |
...
```sh
npm install form-please @tanstack/react-form @tanstack/react-query @floating-ui/react @heroicons/react zod
```
...
`setValue` replaces the complete array when a selection changes. TanStack Form
...
Read [Controls and slots](/controls) for the control contract. Read
