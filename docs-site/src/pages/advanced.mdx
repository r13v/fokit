...
| Apply one update to multiple fields | `form.api.setValues(values)` |
...
Requires React Hook Form 7.74.0 or newer.

Use `setValues` when one action changes multiple fields. One call updates the
fields and applies the same dirty, touch, and validation options to each value.
...
new document baseline. Use `reset(nextInput)` for that operation.

## Save a draft with a subscription

Requires React Hook Form 7.55.0 or newer.
...
Requires React Hook Form 7.55.0 or newer. This version is the minimum version
that Form, Please supports.
