...
Generated field paths use TanStack bracket syntax. The title above can resolve
to `speakers[2].sessions[0].title`.
...
later paths. Put a durable ID in the schema when state must survive reordering.
...
fields keep their TanStack Form values.
...
Use dot syntax for object properties and bracket syntax for array indexes:
...
Use direct TanStack composition outside `kit.Fields` when content needs the
